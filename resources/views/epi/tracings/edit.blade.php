    ...
    <div class="card">
        <div class="card-body">
            ...
            <div class="form-row">
                ...
                <fieldset class="form-group col-12 col-sm-6">
                    ...
                    <input type="text" class="form-control" name="result_observation" id="for_observation_final" value="{{old('result_observation',$tracing->result_observation)}}" autocomplete="off">
                </fieldset>
            </div>
            <hr>
            ...
            @for ($i = 1; $i <= 4; $i++) <div class="form-row">
                <fieldset class="form-group col-8 col-sm-5 col-md-3 col-lg-2">
                    ...
                    <input type="date" class="form-control" name="delivery_of_result_{{$i}}" id="for_delivery_of_result_{{$i}}" value="{{ old('delivery_of_result_' . $i, $tracing->{'delivery_of_result_' . $i}) }}">
                </fieldset>
                ...
                <fieldset class="form-group col-9 col-sm-5 col-md-4 col-lg-3">
                    ...
                    <select name="mechanism_of_result_{{$i}}" id="for_mechanism_of_result_{{$i}}" class="form-control">
                        <option></option>
                        <option value="Pendiente" {{ old('mechanism_of_result_' . $i, $tracing->{'mechanism_of_result_' . $i}) == 'Pendiente' ? 'selected' : '' }}>
                            Pendiente
                        </option>
                        <option value="Llamada telefónica" {{ old('mechanism_of_result_' . $i, $tracing->{'mechanism_of_result_' . $i}) == 'Llamada telefónica' ? 'selected' : '' }}>
                            Llamada telefónica
                        </option>
                        <option value="Correo electrónico" {{ old('mechanism_of_result_' . $i, $tracing->{'mechanism_of_result_' . $i}) == 'Correo electrónico' ? 'selected' : '' }}>
                            Correo electrónico
                        </option>
                        <option value="Visita domiciliaria" {{ old('mechanism_of_result_' . $i, $tracing->{'mechanism_of_result_' . $i}) == 'Visita domiciliaria' ? 'selected' : '' }}>
                            Visita domiciliaria
                        </option>
                        <option value="Centro de salud" {{ old('mechanism_of_result_' . $i, $tracing->{'mechanism_of_result_' . $i}) == 'Centro de salud' ? 'selected' : '' }}>
                            Centro de salud
                        </option>
                        <option value="Carta certificada" {{ old('mechanism_of_result_' . $i, $tracing->{'mechanism_of_result_' . $i}) == 'Carta certificada' ? 'selected' : '' }}>
                            Carta certificada
                        </option>
                    </select>
                </fieldset>
                ...
                <fieldset class="form-group col-12 col-sm-6">
                    <label for="for_observation_of_result_{{$i}}">Observación</label>
                    <input type="text" class="form-control" name="observation_of_result_{{$i}}" id="for_observation_of_result_{{$i}}" value="{{ old('observation_of_result_' . $i, $tracing->{'observation_of_result_' . $i}) }}" autocomplete="off">
                </fieldset>
        </div>
        @endfor
        ...
        <div class="form-row">
            ...
            <fieldset class="form-group col-12 col-sm-6 col-md-5 col-lg-3 order-2 order-lg-2">
                ...
                <input type="datetime-local" class="form-control" name="next_control_at" id="for_next_control_at" required value="{{ ($tracing->next_control_at) ? $tracing->next_control_at->format('Y-m-d\TH:i:s') : '' }}">
            </fieldset>
            ...
            <fieldset class="form-group col-12 col-sm-12 col-md-9 col-lg-4 order-5 order-lg-4">
                ...
                <select name="establishment_id" id="for_establishment_id" class="form-control" required>
                    ...
                    <option value="{{$organization->id}}" {{ ($tracing->establishment_id == $organization->id) ? 'selected' : '' }}>
                        {{$organization->alias??''}}
                    </option>
                    @endforeach
                </select>

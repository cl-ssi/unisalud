<x-filament::widget>
    <x-filament::section>
        <div id="map"></div>
        <div class="hidden" id="dev-data" data-markers="{{json_encode($markers)}}"
    ...
    <script>
        document.addEventListener('livewire:initialized', function () {
            ...
            Promise.all([
                ...
                fetch("/json/Cuenca_Quebrada_Seca_Alto_Hospicio.geojson").then(res => res.json())
            ]).then(([lineaGeo, cotaGeo, aluvionGeo, cuencaGeo]) => {
                lineaData = lineaGeo;
                cotaData = cotaGeo;
                aluvionData = aluvionGeo;
                cuencaData = cuencaGeo;
                setupMap()
            }).catch(error => {
            ...
            function setupMap() {
                ...
                geoJsonLayersRef.cuenca = L.geoJSON(cuencaData, {
                    style: {
                        color: '#9933ff',
                        weight: 2,
                        opacity: 0.7
                ...
                const overlayLayers = {
                    ...
                    'Zona de cuenca': geoJsonLayersRef.cuenca,
